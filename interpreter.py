...
class TokenType:
    ...
    PLUS = 'PLUS'
    MINUS = 'MINUS'
    MUL = 'MUL' # Новий токен множення
    DIV = 'DIV' # Новий токен ділення
    LPAREN = 'LPAREN' # Новий токен відкрита дужка
    RPAREN = 'RPAREN' # Новий токен закрита дужка
...
class Lexer:
    ...
    def skip_letters(self):
        """ Пропоскаємо літери і слова"""
...
class Parser:
    ...
    #Шкаємо ціле число
        if token.type == TokenType.INTEGER:
            ...
            return Num(token)
    # У разі якщо під час парсингу зустрічаємо відкриту дужку - імо її
        elif token.type == TokenType.LPAREN:
            self.eat(TokenType.LPAREN)
            node = self.expr() # рекурсивно викликакаємо метод expr() який відповідає за +, -
    ...
    def term(self): # Метод для множення діленні. він іде перший така я множення, ділення приорітет
        node = self.factor() # Вмкликаємо метод factor() у разі якщо були дужки.
...
def main():
    while True:
        try:
            ...
            print(f" [bold green]Результат обчислення: [/bold green]{result}")
